...
# Set up file handler if in DEBUG mode
if logger.isEnabledFor(logging.DEBUG):
    file_handler = logging.FileHandler(repo_root / "logs" / "pipeline.log")
...
def lambda_handler(event, context):
    ...
    # 1. Get bucket and key from event, and other flags frm env vars
    ...
    # the putput dir is set in the env vars
    ...
    if key.startswith("xml_input/") and key.endswith(".xml"):
        ...
        if not action or not key:
            logger.error("Missing action or file in event: event=%s", event)
            return {"status": "error", "message": "Missing action or file in event"}

        if action != "run_pipeline" or not key.endswith(".xml"):
            logger.error("Invalid action or file type: action=%s file=%s", action, key)
            return {"status": "error", "message": "Invalid action or file type"}

        # 2. Determine source: local file or S3 download
        xml_path_to_convert = None
        tmp_path = None

        # Download from S3 unless in DEBUG mode
        ...
        if save_intermediate and logger.isEnabledFor(logging.DEBUG):
            ...
            with log_timing("Creating tarballs", logger):
                ...
                for level_name, files in jsons_by_level.items():
                    ...
                    try:
                        ...
                        # Write locally if in DEBUG mode
                        if logger.isEnabledFor(logging.DEBUG):
                        ...
                        # Upload to S3 (always, unless in debug-only mode)
                        if bucket and not logger.isEnabledFor(logging.DEBUG):
...
# Run the handler locally if in DEBUG mode
if logger.isEnabledFor(logging.DEBUG):
